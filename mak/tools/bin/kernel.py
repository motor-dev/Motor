...
class DeclarationVisitor(ast.Visitor):
    ...
    def __init__(self, namespace: object, decl_specifier_seq: ast.DeclSpecifierSeq) -> None:
...
def write_cc(capitalized_name, namespace, out_cc, full_name):
    ...
    if namespace[3]:
        ...
        for kernel_name, parameters in namespace[3].items():
            ...
            out_cc.write(
                ...
                '        );\n'
                '    ref< ::Motor::KernelScheduler::Producer::Runtime > result =\n'
                '        ref< ::Motor::KernelScheduler::Producer::Runtime >::create(\n'
...
def write_hh(namespace, out_hh, full_name):
    ...
    if namespace[1]:
        ...
        for using_declaration in namespace[1]:
            for reference in using_declaration.references:
                sr = utils.StringRef()
                reference.accept(sr)
