...
local default_compilers = {}
local default_platforms = {}
for _, c in ipairs(context:search(context.path, 'compiler/*.lua')) do
    table.insert(default_compilers, c:basename())
end
for _, p in ipairs(context:search(context.path, 'target/*.lua')) do
    table.insert(default_platforms, p:basename())
end

compilers = context.settings.compiler or default_compilers
platforms = context.settings.platform or default_platforms
...
context:declare_command("list_toolchains", "list_toolchains", env, true)
