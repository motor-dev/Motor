...
def _vscode_path_from(path: Union[str, waflib.Node.Node], node: waflib.Node.Node) -> str:
...
class vscode(vscode_common):
    ...
    def write_workspace(self) -> None:
                ...
                for g in self.groups:
                    for tg in g:
                        ...
                        if len(commands) != mark:
                            ...
                            tg_includes = [_vscode_path_from(i, self.srcnode) for i in tg_includes]
        ...
        try:
            tasks = json.loads(json_minify(waflib.Utils.readf(tasks_file.abspath(), 'r')))
        except IOError:
        ...
        try:
            launch_config_content = json_minify(waflib.Utils.readf(launch_file.abspath(), 'r'))
                        ...
                        launch_configs['configurations'].append(
                            {
                                ...
                                'cwd':
                                    '${workspaceFolder}/${input:motor-Prefix}/${input:motor-Variant}/'
                                    '${input:motor-Deploy_RunBinDir}',
                                'preLaunchTask': '${command:cmake.tasksBuildCommand}'
                            }
                        )
