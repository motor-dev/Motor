...
use super::{Command, CommandOutput, CommandStatus, SerializedHash};
use std::fmt;
...
use crate::environment::{Environment, ReadWriteEnvironment, ReadWriteEnvironmentSequenceSeed, SerializedReadWriteEnvironment};
use crate::node::Node;
        ...
        impl<'de> DeserializeSeed<'de> for CommandCacheSeed {
                ...
                D: Deserializer<'de>,
            {
                ...
                impl<'de> Visitor<'de> for CommandCacheVisitor {
                    ...
                    fn visit_seq<V>(mut self, mut seq: V) -> Result<Vec<Command>, V::Error>
                    where
                        V: SeqAccess<'de>,
                    {
        ...
        D: Deserializer<'de>,
    {
                        ...
                        E: Error,
                    {
                        match value {
                            ...
                            _ => Err(Error::unknown_field(
                                value,
                                &["environments", "commands", "options", "tools", "stored_hash", "groups"],
                            )),
                        }
                    }
                }
        ...
        impl<'de, 'a> Visitor<'de> for CommandOutputVisitor<'a> {
                ...
                V: MapAccess<'de>,
            {
                ...
                let groups =
                    groups.ok_or_else(|| Error::missing_field("groups"))?;
                Ok(Some(CommandOutput {
                    environments,
                    commands,
                    options,
