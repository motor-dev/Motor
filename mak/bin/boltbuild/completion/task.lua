...
---@param always_run boolean? Optional. If set to `true`, tasks using this driver will always execute, regardless of up-to-date checks.
function Context:command_driver(name, color, command, run_before, always_run)
end
...
---@param run_before (string|string[])? Optional. The name of other drivers that this driver runs before. All tasks using this driver will be scheduled before tasks using the specified drivers.
---@param always_run boolean? Optional. If set to `true`, tasks using this driver will always execute, regardless of up-to-date checks.
function Context:dependency_driver(name, color, command, run_before, always_run)
end
...
---@param always_run boolean? Optional. If set to `true`, tasks using this driver will always execute, regardless of up-to-date checks.
function Context:lua_driver(name, color, script, run_before, always_run)
end

---Executes the specified driver with the provided inputs and outputs, using the given environment settings.
---This method can be used to execute a driver without generating a task. This can be used during a configuration phase
---to check the availability of a program or a flag.
---All input nodes will be added to the list of files tracked by the context to determine if the command is up-to-date.
---Additionally, all file dependencies of the driver will be added to the list of files tracked by the context.
---If any direct or indirect input file changes after the command is executed, the context will be considered out-of-date.
---
---@param driver string The name of the driver to execute.
---@param inputs Node[]|Node A `Node` or list of `Node` objects to be used as inputs for the driver.
---@param outputs Node[]|Node A `Node`or list of `Node` objects to be created or modified by the driver.
---@param env Environment The environment settings to use for the execution.
---@return number #Exit code
---@return string #Combined output and error logs
function Context:run_driver(driver, inputs, outputs, env)
end
