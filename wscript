...
def options(option_context):
    ...
    option_context.motornode = option_context.path
    option_context.recurse('mak/build_framework/options/options.py')
...
def configure(configuration_context):
    ...
    configuration_context.recurse('mak/build_framework/configure/configure.py')
...
def multiarch_setup(configuration_context):
    ...
    configuration_context.motornode = configuration_context.path
    configuration_context.recurse('mak/build_framework/setup/setup.py')


def setup(configuration_context):
    "setup a platform environment in the current configuration context"
    configuration_context.motornode = configuration_context.path
    configuration_context.recurse('mak/build_framework/setup/setup.py', once=False)
...
def build(build_context):
    ...
    build_context.recurse('mak/build_framework/build/build.py')
...
def run(build_context):
    ...
    build_context.motornode = build_context.path
    build_context.recurse('mak/build_framework/run/run.py')


def debug(build_context):
    "start a debugging session on a device"
    build_context.motornode = build_context.path
    build_context.recurse('mak/build_framework/debug/debug.py')
