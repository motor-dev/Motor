...
#ifndef MOTOR_MINITL_INL_HASHMAP_HH
#define MOTOR_MINITL_INL_HASHMAP_HH
    ...
    , m_count(other.m_count)
{
    ...
    if(m_count)
    {
        list_iterator my_it = m_items.begin();
    ...
    if(m_index)
    {
        for(index_item_t* it = m_index.begin(); it != m_index.end() - 1; ++it)
        {
            ...
            while(object != (it + 1)->second)
            {
                ...
                m_itemPool.release(&static_cast< item& >(*item_to_delete));
            }
            it->~index_item_t();
        }
...
void hashmap< KEY, VALUE, HASH >::grow(u32 size)
{
    ...
    pool< item >                     new_pool(m_index.arena(), size);
    allocator::block< index_item_t > new_index(m_index.arena(), size + 1);
    intrusive_list< empty_item >     new_list;

    list_iterator current = new_list.begin();
    for(index_item_t* it = new_index.begin(); it != new_index.end(); ++it)
    {
        new(it) index_item_t;
        current = it->second = new_list.insert(current, it->first);
    }
