...
byte* SystemAllocator::platformReserve(u32 size)
{
    ...
    motor_assert(result, "failed to reserve memory for %d bytes: %s" | size | strerror(errno));
#if !MOTOR_ENABLE_MEMORY_DEBUGGING
    int failed = mprotect(result, cacheAhead(), PROT_READ | PROT_WRITE);
    motor_forceuse(failed);
    motor_assert(failed == 0, "failed to reserve memory for %d bytes at offset %d: %s" | size | 0
                                  | sys_errlist[errno]);
#endif
    return result;
}
...
{
    motor_assert((intptr_t)ptr % platformPageSize() == 0,
                 "pointer %p is not aligned on a page boundary (page size = %d)" | ptr
                     | platformPageSize());
    motor_assert(begin % platformPageSize() == 0,
                 "offset %d is not aligned on a page boundary (page size = %d)" | begin
                     | platformPageSize());
    motor_assert(end % platformPageSize() == 0,
                 "offset %d is not aligned on a page boundary (page size = %d)" | end
                     | platformPageSize());
    int failed = mprotect((char*)ptr + begin + cacheAhead(), end - begin, PROT_READ | PROT_WRITE);
    motor_forceuse(failed);
    motor_assert(failed == 0, "failed to commit memory for %d bytes at offset %d: %s"
...
void SystemAllocator::platformRelease(byte* ptr, u32 begin, u32 end)
{
    ...
    motor_forceuse(failed);
    motor_assert(failed == 0, "failed to release memory for %d bytes at offset %d: %s"
                                  | (end - begin) | begin | strerror(errno));
}
...
void SystemAllocator::platformFree(byte* ptr, u32 size)
{
    ...
    motor_forceuse(failed);
    motor_assert(failed == 0, "failed to unmap memory for %d bytes: %s" | size | strerror(errno));
}
