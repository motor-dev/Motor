...
BULLET_BINARIES = 'https://github.com/motor-dev/Motor/releases/download/prebuilt/bullet-2.87-%(platform)s-%(arch)s-%(abi)s.tgz'
...
def setup_system(conf):
    if 'windows' not in conf.env.VALID_PLATFORMS and conf.env.CXX_NAME != 'sun' and conf.check_lib(
...
def setup_prebuilt(conf):
    try:
        ...
        if not conf.check_package('motor.3rdparty.physics.bullet', bullet_node, var='bullet'):
