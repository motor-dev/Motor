...
{
    if(range1.begin > range1.end) return false;
    if(range2.begin > range2.end) return false;
...
template < Endianness endianness >
DwarfModule::Buffer< endianness >&
DwarfModule::Buffer< endianness >::operator>>(Dwarf::offset_t& value)
{
    if(m_64)
    {
        value.value = ByteSwap< u64, endianness >::byteswap(
...
template < Endianness endianness >
DwarfModule::Buffer< endianness >&
DwarfModule::Buffer< endianness >::operator>>(Dwarf::uleb128_t& value)
{
    int i       = 0;
    value.value = 0;
    do
...
bool DwarfModule::resolve(u64 address, Symbol& symbol) const
{
    ...
    {
        AddressRange r;
        r.begin = r.end            = address;
        UnitMap::const_iterator it = m_units.find(r);
        if(it != m_units.end())
        {
            fillSymbol(symbol, address, m_moduleName[m_moduleName.size() - 1].c_str(),
                       it->second.name, "", 0);
        }
        else
        {
...
template < Endianness endianness >
bool DwarfModule::readAbbreviation(Buffer< endianness >&                  buffer,
...
template < Endianness endianness >
bool DwarfModule::fillNode(Buffer< endianness >& buffer, CompilationUnit& r,
                           ...
                           const minitl::vector< Dwarf::Abbreviation >& abbreviations, u8 ptrSize)
{
    ...
    for(unsigned i = 0; i < abbrev.propertyCount; ++i)
    {
        ...
        switch(type)
        {
        case Dwarf::Type_addr: buffer.m_position += ptrSize; break;
        ...
        case Dwarf::Type_ref2: buffer.m_position += 2; break;

        case Dwarf::Type_data4:
        case Dwarf::Type_ref4: buffer.m_position += 4; break;

        case Dwarf::Type_data8:
        case Dwarf::Type_ref8: buffer.m_position += 8; break;
        ...
        case Dwarf::Type_ref1: buffer.m_position++; break;

        case Dwarf::Type_flag: buffer.m_position++; break;
...
template < Endianness endianness >
bool DwarfModule::readInfos(Buffer< endianness >& buffer, UnitMap& units,
                            const minitl::vector< Dwarf::Abbreviation >& abbreviations, u8 ptrSize)
{
    Dwarf::uleb128_t l;
